...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/internal/email-test" && request.method === "POST") {
      if (url.searchParams.get("token") !== "ancestrytrip-email-test-20260926-7f4c9b2a") {
        return json({ error: "Not found." }, 404);
      }

      const tripId = crypto.randomUUID().replaceAll("-", "");
      ...
      try {
        await env.DB.prepare(`
          INSERT INTO ancestry_trips (
            id, email, first_name, last_name, ancestral_place, preview_json
          )
          VALUES (?, ?, ?, ?, ?, ?)
        `)
          .bind(
        ...
        return json({
          ok: result?.email_status === "sent",
          emailStatus: result?.email_status || null,
          emailSentAt: result?.email_sent_at || null,
          emailError: result?.email_error || null,
        });
      } finally {
        await env.DB.prepare(
          "DELETE FROM ancestry_payments WHERE id = ?",
        )
          .bind(paymentId)
          .run();

        await env.DB.prepare(
          "DELETE FROM ancestry_trips WHERE id = ?",
        )
          .bind(tripId)
          .run();
      }
